(function ($) {
	...
	***************************************************/
	var createMap = function(key, name, title){
	   ...
	   $('.view-reportes.view-display-id-page #'+key+'-wrapper input'+', .view-reportes.view-display-id-page_8 #'+key+'-wrapper input'+', .view-reportes.view-display-id-page_9 #'+key+'-wrapper input').css('display','none');
	   $('.view-reportes.view-display-id-page #'+key+'-wrapper'+', .view-reportes.view-display-id-page_8 #'+key+'-wrapper'+', .view-reportes.view-display-id-page_9 #'+key+'-wrapper').css({'width':'100%'});
	   ...
	   ***************************************************/
		...
		//New categories - Estado del sitio
		var no_adjud = new OpenLayers.Rule({
			filter: new OpenLayers.Filter.Comparison({
				type: OpenLayers.Filter.Comparison.GREATER_THAN,
				property: "no_adjudicado",
				value: 0,
			 }),
			 symbolizer: {
				fillColor: "rgb(255, 112, 40)",
				fillOpacity: 1,
				strokeColor: "rgb(255,255,255)",
				strokeOpacity: 0.3,
				strokeWidth: 2,
		...
		var conectividad_estatal = new OpenLayers.Rule({
			filter: new OpenLayers.Filter.Comparison({
				type: OpenLayers.Filter.Comparison.GREATER_THAN,
				property: "conectividad_estatal",
				value: 0,
			 }),
			 symbolizer: {
		...
		var estado_inmueble = new OpenLayers.Rule({
			filter: new OpenLayers.Filter.Comparison({
				type: OpenLayers.Filter.Comparison.GREATER_THAN,
		...
		switch(getUrlVar('style')){
			...
			case "Puntos": 
			default:
				//$rules=[point,connec,adjud,red,connecst, licitado, infraestructuraEstilo,cobre,fibra, puntos_interconexion,registros_fo,derecho_via,poligono_espacios];
				$rules=[point, estado_inmueble, conectividad_estatal, redes, no_adjud, adjud, connec, infraestructuraEstilo,cobre,fibra, puntos_interconexion,registros_fo,derecho_via,poligono_espacios];
				$strategies=[];
			break;
		}
		...
		"select": new OpenLayers.Style({
	     ...
	     mapsLayers[0] = new OpenLayers.Layer.Google("Google Street", {sphericalMercator: true});
	     
	     
	     //new OpenLayers.Layer.Google("Google Streets",{type: google.maps.MapTypeId.TERRAIN, numZoomLevels: 21 });
	     
	     
	     mapsLayers[1] = new OpenLayers.Layer.Google("Google Hybrid",{type: google.maps.MapTypeId.HYBRID,numZoomLevels: 22 });
		 ...
		 $('.view-reportes.view-display-id-page #'+key+'-wrapper'+', .view-reportes.view-display-id-page_8 #'+key+'-wrapper'+', .view-reportes.view-display-id-page_9 #'+key+'-wrapper').append('<legend><span class="fieldset-legend">'+title+'</span></legend>\
	    ...
	    $(".page-reportes select").multiselect({ enableFiltering: true });
		  ...
		  $('#ctrl-delete-'+name).click(function() {
            ...
            $.each(cartaro_reportes_results.response.docs, function(i, item) {	
				...
				if(item.geo_field_ubicacion!=undefined){
					...
					if(item.bm_field_instalado != undefined) {
						fea.attributes.instalado = item.bm_field_instalado[0];
					}
					
					if(item.bs_field_adjudicado != undefined) {
						fea.attributes.adjudicado = item.bs_field_adjudicado;
					}
					
					if(item.bs_field_no_adjudicado != undefined) {
						fea.attributes.no_adjudicado = item.bs_field_no_adjudicado;
					}
					
					var red_10k = (item.bs_field_red_10k != undefined) ? item.bs_field_red_10k : false;
					var red_11k = (item.bs_field_red_11k != undefined) ? item.bs_field_red_11k : false;
					var red_23k = (item.bs_field_red_23k != undefined) ? item.bs_field_red_23k : false;
					var red_terrestres = (item.bs_field_red_operadores_terrestr != undefined) ? item.bs_field_red_operadores_terrestr : false;
					var red_40 = (item.bs_field_40_redes != undefined) ? item.bs_field_40_redes : false;
					var red_mc2 = (item.bs_field_mc2_adjudicado != undefined) ? item.bs_field_mc2_adjudicado : false;
					var red_bicentenario = (item.bs_field_bicentenario != undefined) ? item.bs_field_bicentenario : false;
					
					if(red_10k || red_11k || red_23k || red_terrestres || red_40 || red_mc2 || red_bicentenario) {
						fea.attributes.redes = true;
					}
					
					var conectado_estatal = (item.bs_field_conectado_con_red_estat != undefined) ? item.bs_field_conectado_con_red_estat : false;
					var conectividad = (item.sm_field_conectividad_sito2 != undefined) ? item.sm_field_conectividad_sito2[0] : false;
					
					if(conectado_estatal || conectividad) {
						fea.attributes.conectividad_estatal = true;
					}
					
					var energia_electrica = (item.sm_field_energia_electrica_sitio != undefined) ? item.sm_field_energia_electrica_sitio[0] : false; 
					var techo_lamina = (item.sm_field_techo_lamina != undefined) ? item.sm_field_techo_lamina[0] : false;
					var techo_carton = (item.sm_field_techo_carton != undefined) ? item.sm_field_techo_carton[0] : false;
					var particular_privado = (item.sm_field_inmueble_part_o_privado != undefined) ? item.sm_field_inmueble_part_o_privado[0] : false;
					var inmueble_construccion = (item.sm_field_inmueble_construccion != undefined) ? item.sm_field_inmueble_construccion[0] : false;
					
					if(!energia_electrica || techo_lamina || techo_carton || particular_privado || inmueble_construccion) {
						fea.attributes.estado_inmueble = true;
					}
			...
			if(mapsLayers[3].features[i].cluster!=null)
				if(mapsLayers[3].features[i].cluster.length==1){
					 mapsLayers[3].features[i].attributes.cerca=mapsLayers[3].features[i].cluster[0].attributes.cerca;
	  				 mapsLayers[3].features[i].attributes.instalado=mapsLayers[3].features[i].cluster[0].attributes.instalado;
  ...
  attach: function (context, settings) {
   ...
   $.each(Drupal.settings.geo_filters, function( key, name ) {
                   ...
                   if($('#'+key).html()!=null){
	                     ...
	                     $(".view-reportes.view-display-id-page #edit-geo-field-ubicacion-wrapper, .view-reportes.view-display-id-page_8 #edit-geo-field-ubicacion-wrapper, .view-reportes.view-display-id-page_9 #edit-geo-field-ubicacion-wrapper").append('<div id="nomenclatura-wrapper" style="position:relative; top:-10px"><ul class="nomeclatura"><li class="title">Simbología:</li>'+
					...
					'<ul id="estado" class="nomeclatura">'+
							...
							'<li><img  alt="icon" src="/sites/all/modules/maps_mc/images/colores/verde.png" />Instalado</li>'+
							'<li><img  alt="icon" src="/sites/all/modules/maps_mc/images/colores/amarillo.png" />Adjudicado</li>'+
							'<li><img  alt="icon" src="/sites/all/modules/maps_mc/images/colores/naranja.png" />No adjudicado</li>'+
							'<li><img  alt="icon" src="/sites/all/modules/maps_mc/images/colores/azul_marino.png" />Redes CSIC</li>'+
							'<li><img  alt="icon" src="/sites/all/modules/maps_mc/images/colores/azul_cielo.png" />Red estatal y Conectividad</li>'+
							'<li><img  alt="icon" src="/sites/all/modules/maps_mc/images/colores/gris.png" />Sin condiciones de elegibilidad</li>'+
					'</ul>'+
				    '<ul id="numero-proveedores" class="nomeclatura">'+
							'<li class="title">Número de proveedores:</li>'+
