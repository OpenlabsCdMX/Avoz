(function ($) {
	...
	***************************************************/
	var createMap = function(key, name, title){
	   ...
	   $('.view-reportes.view-display-id-page_1 #'+key+'-wrapper input').css('display','none');
	   $('.view-reportes.view-display-id-page_1 #'+key+'-wrapper').css({'width':'100%'});
...
/*New styles*/
		...
		var distance = new OpenLayers.Rule({
			filter: new OpenLayers.Filter.Comparison({
				type: OpenLayers.Filter.Comparison.GREATER_THAN,
				property: "distance",
				value: 0
		...
		switch(getUrlVar('style')){
			...
			case "Densidad": 
				$rules=[density,infraestructuraEstilo,cobre,fibra, puntos_interconexion,registros_fo,derecho_via,poligono_espacios,distance];
				$strategies=[];
			break;
			case "Cluster":
				$rules=[point, connec, red, connecst,licitado, lowRule, middleRule, highRule,infraestructuraEstilo,cobre,fibra, puntos_interconexion,registros_fo,derecho_via,poligono_espacios,distance];
				$strategies=[
			...
			case "Puntos": 
			default:
				$rules=[point,connec,red,connecst,licitado,infraestructuraEstilo,cobre,fibra, puntos_interconexion,registros_fo,derecho_via,poligono_espacios,distance];
				$strategies=[];
			break;
		}
		...
		"select": new OpenLayers.Style({
	     ...
	     mapsLayers[18] = new OpenLayers.Layer.Vector("Enlaces", {
			projection: geographic,
		    styleMap: new OpenLayers.StyleMap(style)
   	     });
		 ...
		 $('.view-reportes.view-display-id-page_1 #'+key+'-wrapper').append('<legend><span class="fieldset-legend">'+title+'</span></legend>\
	    ...
	    $(".page-reportes-sitios select").multiselect({ enableFiltering: true });
	     ...
	     ***************************************************/  
		  $('#ctrl-delete-'+name).click(function() {
            ...
            $.each(cartaro_reportes_results, function(i, item) {	
				...
				if(item.geo_field_ubicacion!=undefined){
					...
					if(item.parent_id != undefined){
						//p1 = item.geo_field_ubicacion[0].replace("POINT(","").replace(")","").split(" ");
						p1 = new OpenLayers.Format.WKT(in_options).read(item.geo_field_ubicacion[0]);
						//p2 = cartaro_reportes_results[item.parent_id].geo_field_ubicacion[0].replace("POINT(","").replace(")","").split(" ");

						p2 = new OpenLayers.Format.WKT(in_options).read(cartaro_reportes_results[item.parent_id].geo_field_ubicacion[0]);
						var points = new Array(p1.geometry,p2.geometry);
						var line = new OpenLayers.Geometry.LineString(points);
						var lineFeature = new OpenLayers.Feature.Vector(line);
						lineFeature.attributes.distance=((item._dist_)*1000).toFixed(4) ;
						features_lines.push(lineFeature);
			...
			mapsLayers[18].addFeatures(features_lines);
	     ...
	     ****************************************************/
           ...
           if(map[name].getZoom()>="19")	           
	           mapsLayers[18].setVisibility(true);
           else
        	   	mapsLayers[18].setVisibility(false);
	  	...
	  	$.each(Drupal.settings.geo_filters, function( key, name ) {
                   if($('#'+key).html()!=null){
                      name= name.split('|');                   
	                     ...
	                     $(".view-reportes.view-display-id-page_1 #edit-geo-field-ubicacion-wrapper").append('<div id="nomenclatura-wrapper" style="position:relative; top:-10px"><ul class="nomeclatura"><li class="title">Simbología:</li>'+
